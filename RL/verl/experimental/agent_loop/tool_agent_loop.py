...
@register("my_tool_agent_with_user")
class UserToolAgentLoop(ToolAgentLoop):
    ...
    def init_class(cls, config, tokenizer, processor, **kwargs):
        if cls._class_initialized:
            return
        cls._class_initialized = True
        print("Performing class-level UserToolAgentLoop initialization")
    ...
    async def _handle_processing_tools_state(self, agent_data: AgentData) -> AgentState:
        ...
        decoded_text = await self.loop.run_in_executor(
            None, lambda: self.tokenizer.decode(agent_data.prompt_ids)
        )

        tasks = []
        tool_call_names = []
        for tool_call in agent_data.tool_calls[: self.max_parallel_calls]:
            tasks.append(self._call_tool(tool_call, agent_data.tools_kwargs, decoded_text))
            tool_call_names.append(tool_call.name)
        ...
        for tool_response, tool_reward, _ in responses:
            ...
            if tool_response.image or tool_response.video:
                ...
                # Text-only content
                if len(tool_call_names) == 0 or tool_call_names[0] == "mock_user":
    ...
    async def _call_tool(
        self, tool_call: FunctionCall, tools_kwargs: dict[str, Any], history_messages: str
    ) -> tuple[ToolResponse, float, dict]:
        """Call tool and return tool response."""
        tool, instance_id = None, None
        try:
            ...
            if tool_name in self.tools:
                ...
                tool_descriptions = re.findall(r"<tools>(.*?)</tools>", history_messages, re.DOTALL)[-1]
                # if tool_name == "none" or tool_name is None:
                #     breakpoint()
                if tool_name not in tool_descriptions and tool_name != "mock_user":
                    # logger.warning(f"Error when executing tool: tool {tool_name} not found")
                    return (
                        ToolResponse(
                            text=f"Error when executing tool: tool {tool_name} not found",
                        ),
                        0.0,
                        {},
                    )
                
                tool = self.tools["mock_tool"]
                mock_tool_args = {"name": tool_name, "arguments": json.loads(tool_call.arguments)}
                tool_name = "mock_tool"
                tool_args = {"mock_tool_name_and_args": json.dumps(mock_tool_args), "history_messages": history_messages}
        ...
        tool_response_text = tool_execution_response.text if tool_execution_response else ""
        if tool_response_text and len(tool_response_text) > self.max_tool_response_length:
            if self.tool_response_truncate_side == "left":
                tool_response_text = tool_response_text[: self.max_tool_response_length] + "...(truncated)"
            elif self.tool_response_truncate_side == "right":
