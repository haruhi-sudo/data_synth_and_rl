...
def format_trajectory(trajectory: List[Dict]) -> str:
    """Format a solution trajectory into readable text for evaluation."""
    lines = []
...
def main():
    ...
    parser.add_argument('--config', type=str, default='configs/rubrics.yaml',
    ...
    model_api_config = config["api_configs"][model_name]
    api_base = os.getenv(model_api_config["api_base"])
    api_key = os.getenv(model_api_config["api_key_env"])
    ...
    def process_task(task_id: str) -> Tuple[str, bool, Optional[str]]:
        try:
