...
def mock_tools_node(state: AgentState, config: RunnableConfig):
    ...
    # tool_response, new_bg_introduced = mock_tool_response(cfg, tool_call, tools_description, tool_call_history, key_expected_tool_return=state["key_expected_tool_return"])
    tool_response, new_bg_introduced = mock_tool_response(cfg, tool_call, tools_description, solve_history[1:], key_expected_tool_return=state["key_expected_tool_return"])
...
if __name__ == "__main__":
    ...
    # Example usage
    with open("output/virtual_tool_use_v3.1.jsonl", 'r', encoding='utf-8') as f:
        tasks = [json.loads(line) for line in f][1:]

    for task in tasks:
        if os.path.exists(f"output/solve_tool_use_v3/{task['id']}/tool_call_history.json"):
            with open(f"output/solve_tool_use_v3/{task['id']}/tool_call_history.json", 'r', encoding='utf-8') as f:
                tool_call_history = json.load(f)
        else:
            tool_call_history = []
