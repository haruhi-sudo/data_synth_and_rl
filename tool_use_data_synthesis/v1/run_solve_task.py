...
from v1.graph.solve_task import run_agent

def main():
    parser = argparse.ArgumentParser(description='Run v1 solve tasks from a JSONL file')
    parser.add_argument('--config', type=str, default='v1/configs/solve_task.yaml',
                       help='Path to the configuration file (default: v1/configs/solve_task.yaml)')
    ...
    def process_single_task(task_data: Dict[str, Any], config: Dict[str, Any]) -> bool:
        try:
            ...
            new_task: dict[str, Any] = {
                "id": task_data["id"],
                "fuzzy_task": task_data["fuzzy_task"],
                "checked_tools": task_data["checked_tools"],
            }
            run_agent(seed_info=new_task, run_config=config)
            return True
        except Exception as e:
